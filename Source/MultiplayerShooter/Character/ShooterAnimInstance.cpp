...
void UShooterAnimInstance::NativeUpdateAnimation(float DeltaTime)
{
	...
	if (bWeaponEquipped && EquippedWeapon && EquippedWeapon->GetWeaponMesh() && ShooterCharacter->GetMesh())  
	{
		...
		if (ShooterCharacter->IsLocallyControlled())
		{
			...
			FRotator LookAtRotation = RightHandRotation = UKismetMathLibrary::FindLookAtRotation(RightHandTransform.GetLocation(), RightHandTransform.GetLocation() + (RightHandTransform.GetLocation() - ShooterCharacter->GetHitTarget()));
